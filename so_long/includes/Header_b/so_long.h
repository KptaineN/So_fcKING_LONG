...
/*   Updated: 2025/05/08 01:30:59 by nkiefer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_enemy {
	...
	int spawn_x;
	int spawn_y;
	int step; // 0: haut, 1: retour, 2: gauche, 3: retour, ...
	int glide_dir;
}	t_enemy;
...
typedef struct s_game
{
	...
	int		enemy_timer;
	void	*img_enemy;
	t_enemy *delayed_enemies;   // liste des ennemis à créer quand l’exit est ouverte
	int      delayed_count;     // combien en attente
	int      enemies_spawned;   // flag : 0 → pas encore, 1 → déjà spawnés
	int 	should_exit;
